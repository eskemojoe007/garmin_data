...
def main():
    logger.info(
        '--------------------------sprint_metrics - STARTING SCRIPT--------------------------')
    ...
    parser.add_argument('--ave_thresh', metavar='Average Speed Threshold',
                        ...
                        help='Average speed of a lap must be above this value '
                        'in MPH in order to be identified as a sprint. Default = 9 MPH')
    ...
    parser.add_argument('--scale', metavar='Scale Meters', type=int,
                        help='Scale insoncistent runs to a certain distance (ie 800)')
    parser.add_argument('--goal', metavar='Goal Pace', type=float,
                        help='Goal pace for the sprints (min/mile or 1600m) ie 5.0')
    ...
    ) / data_points['timestamp'].diff().dt.total_seconds() * MS_MPH

    # Auto Determine sprints
    laps['sprint'] = laps.apply(lambda x: app_sprint_get(
        x, ave_thresh=ave_thresh * UREG('mph'), max_thresh=max_thresh * UREG('mph')), axis=1)
    ...
    # seconds
    data_points['seconds'] = (
        data_points['timestamp'] - data_points['timestamp'].loc[0]).map(datetime.timedelta.total_seconds)
    laps['starts_seconds'] = (
        laps['start_time'] - laps['start_time'].loc[0]).map(datetime.timedelta.total_seconds)
    laps['end_seconds'] = (
        laps['timestamp'] - laps['start_time'].loc[0]).map(datetime.timedelta.total_seconds)
    ...
    for i, row in laps.loc[laps['sprint']].iterrows():
        ax.axvspan(row['starts_seconds'], row['end_seconds'],
                   color=CP[2], alpha=0.5, zorder=0)

    # Secondary axis
    get_seconds_series(data_points, 'heart_rate',
                       'seconds').plot(ax=ax2, color=CP[1])
    ...
    # %% Sprint Bar charts
    fig, ax = plt.subplots()
    laps.loc[laps['sprint']].plot.bar(
        x='descrip', y='total_timer_time', legend=False, color=CP[0], ax=ax)
    ax.set_xlabel('')
    ax.set_ylabel('Time [s]')

    for i, time in enumerate(laps['total_timer_time'].loc[laps['sprint']]):
        ax.text(i, time - 2, '{:0.1f}'.format(time),
                rotation=90, ha='center', va='top', color='w')
    if goal is not None:
        goals = []
        x_idx = []
        for i, distance in enumerate(laps['total_distance'].loc[laps['sprint']]):
            x_idx.append(i)
            goals.append((distance*UREG('min/mile').to('sec/m')*goal).magnitude)
        # print(goals, x_idx)
        ax.step(x_idx, goals, linestyle='none', marker='o', color='k')
    fig.savefig(os.path.join(fol, 'bar_times.png'), bbox_inches='tight')
    # end%%

    # %% Sprint Bar chart scaled
    if scale is not None:
        fig, ax = plt.subplots()

        scaled_laps = laps.loc[laps['sprint']].copy()
        scaled_laps['total_timer_time'] = scale / \
            scaled_laps['total_distance'] * scaled_laps['total_timer_time']
        scaled_laps.plot.bar(
            x='descrip', y='total_timer_time', legend=False, color=CP[0], ax=ax)
        ax.set_xlabel('')
        ax.set_ylabel('Time [s]')
        ax.set_title(f'Scaled times to {scale:.0f}m')

        for i, time in enumerate(scaled_laps['total_timer_time']):
            ax.text(i, time - 2, '{:0.1f}'.format(time),
                    rotation=90, ha='center', va='top', color='w')
        if goal is not None:
            ax.axhline((scale*UREG('min/mile').to('sec/m')*goal).magnitude, linestyle='--', color='k')
        fig.savefig(os.path.join(fol, 'bar_times_scaled.png'),
                    bbox_inches='tight')
    # end%%
    ...
    plot_sprints(data_points, laps, 'lap_distance',
                 'speed_mph', ax=axs[0], legend=True)
    plot_sprints(data_points, laps, 'lap_distance',
                 'shitty_speed_mph', ax=axs[1], legend=False)
    if goal is not None:
        axs[0].axhline(1.0/(goal*UREG('min/mile').to('hour/mile')).magnitude, linestyle='--', color='k')
        axs[1].axhline(1.0/(goal*UREG('min/mile').to('hour/mile')).magnitude, linestyle='--', color='k')
    ...
    # %% BPM Over Distance
    fig, ax = plt.subplots(1)
    plot_sprints(data_points, laps, 'heart_rate',
                 'shitty_speed_mph', ax=ax, legend=True, style='o')
    # ax.set_ylim([10,18])
    ax.set_ylabel('Speed [MPH]')
    ax.set_title('Looking for speed and heart rate trends')
    ax.set_xlabel('Heart Rate [BPM]')
    fig.savefig(os.path.join(fol, 'heart_rate_v_speed.png'),
                bbox_inches='tight')
    # end%%

    logger.info(
        '--------------------------sprint_metrics - ENDING SCRIPT--------------------------')
...
def laps_check_overlap(df):
    ...
    got_bad = any((df['start_time'] - df['timestamp'].shift().fillna(
        df['start_time'].iloc[0])) != pd.to_timedelta(0))

    if got_bad:
        fn_bad_laps = 'Bad Laps Information.csv'
        laps[['start_time', 'timestamp']].to_csv(fn_bad_laps)
        logger.warning(
            'We found overlapping laps.  See Laps output: in {}'.format(fn_bad_laps))
...
def app_lap_get(val, laps, laps_start='start_time', laps_end='timestamp'):
    ...
    if len(idx) == 1:
        ...
        logger.warning(
            'For this datapoint, I couldnt find an appropriate lap. This point will be ommited: {}'.format(val))
        return np.nan
...
def plot_sprints(data_df, lap_df, x, y, sprints_only=True, ax=None, **kwargs):
    ...
    if sprints_only:
        filtered_data = data_df.loc[data_df['lap_id'].isin(
            lap_df.index[lap_df['sprint']])]
    else:
    ...
    if legend:
        labels = lap_df['descrip'].loc[lap_df.index.isin(
            filtered_data['lap_id'].unique())].values
        ax.legend(labels)
    return ax
...
def get_input_file(parsed_args, key, real_text=None):
    ...
    if vars(parsed_args)[key] is None:
        logger.debug(
            'You didnt enter a file in the command line for %s...opening dialog' % (key))
