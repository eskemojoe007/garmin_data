...
# %%
tree = ET.parse('activity_2177601737.tcx')
root = tree.getroot()
...
ax.get_figure().savefig('2017_10_23_Figs\Speed_vs_distance.png',bbox_inches='tight')
ax = plot_dfs(dfs,'distance','bpm')
ax.set_ylabel('Heart Rate [BPM]')
ax.set_xlabel('Distance [m]')
ax.get_figure().savefig('2017_10_23_Figs\BPM_vs_distance.png',bbox_inches='tight')
ax = plot_dfs(dfs,'distance','cadence')
ax.set_ylabel('Cadence [Steps/min]')
ax.set_xlabel('Distance [m]')
ax.get_figure().savefig('2017_10_23_Figs\Cadence_vs_distance.png',bbox_inches='tight')
ax = plot_dfs(dfs,'time','distance',ls='none',marker='o')
ax.set_ylabel('Distance [m]')
ax.set_xlabel('Time [s]')
ax.legend()
ax.get_figure().savefig('2017_10_23_Figs\distance_vs_time.png',bbox_inches='tight')
ax = plot_dfs(dfs,'bpm','speed',ls='none',marker='o')
ax.set_ylabel('Speed [MPH]')
ax.set_xlabel('Heart Rate [BPM]')
ax.set_ylim([10,20])
ax.legend()
ax.get_figure().savefig('2017_10_23_Figs\BPM_vs_Sp.png',bbox_inches='tight')
# end%%
...
ax.get_figure().savefig('2017_10_23_Figs\Times_bar.png',bbox_inches='tight')
# end%%
...
for df in dfs:
    ...
    times.append(df['time'].iloc[-1]/df['distance'].iloc[-1]*100.0)
ax = pd.DataFrame(times,index=[range(16)]).plot(kind='bar',legend=False)
ax.set_ylabel('Time [s]')
ax.set_title('Normalized 100m Times for various sprints')

ax.get_figure().savefig('2017_10_23_Figs\Times_bar_normed.png',bbox_inches='tight')
# end%%
